...
class TestRunInterchangeInterventions:
    ...
    def test_basic_intervention_run(self, mock_tiny_lm: Any, model_units_list: Any):
        ...
        with (
            patch(
                "causalab.neural.activations.interchange_mode.prepare_intervenable_model",
                return_value=mock_model,
            ) as mock_prepare,
            patch(
                "causalab.neural.activations.interchange_mode.batched_interchange_intervention"
            ) as mock_batched,
            patch(
                "causalab.neural.activations.interchange_mode.delete_intervenable_model"
            ) as mock_delete,
        ):
    ...
    def test_with_output_scores(self, mock_tiny_lm: Any, model_units_list: Any):
        ...
        with (
            ...
            patch(
                "causalab.neural.activations.interchange_mode.convert_to_top_k"
            ) as mock_convert,
        ):
    ...
    def test_with_tqdm_progress(self, mock_tiny_lm: Any, model_units_list: Any):
        ...
        with (
            ...
            patch("causalab.neural.activations.interchange_mode.tqdm") as mock_tqdm,
        ):
    ...
    def test_empty_dataset(self, mock_tiny_lm: Any, model_units_list: Any):
        """Test behavior with an empty dataset returns empty results."""
        # Extract units from fixture and create InterchangeTarget
        model_units_sublist = model_units_list[0]
        all_units = []
        for units in model_units_sublist:
        ...
        with (
            ...
            # Call the function with an empty dataset - should return empty results
            results = run_interchange_interventions(
                pipeline=mock_tiny_lm,
            ...
            assert results == {"sequences": [[]], "string": [[]]}
